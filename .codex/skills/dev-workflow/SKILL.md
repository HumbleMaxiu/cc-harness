...
- 计划形成后，如果关键方案依赖外部 claim、API 假设或复杂论证，应先触发 `challenger`
- 完成声明如果依赖“应该如此”的推断，而不是明确证据，也应先触发 `challenger`

`challenger` 输出固定的 `CLAIM / CHALLENGE / VERIFICATION / VERDICT` 结构，主 agent 再决定继续、补验证或阻塞回退。
