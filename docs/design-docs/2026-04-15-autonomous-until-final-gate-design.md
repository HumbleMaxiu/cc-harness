...
4. 反馈 memory 记录的是抽象后的问题模式和规则，而不是具体某条 lint 文本
4. 用户只在最终交付时看到：
   - 最终产物
   - 已执行验证
   - 自动修复摘要
...
这样 memory 记录的重点从“等待用户批准”转为“规则化经验 + 自动处理可审计 + 最终确认可追踪”。
...
- 原始 lint / test / review 文本保留在交接文档中；进入 memory 的必须是抽象后的模式和规则

### 5. 风险分级自动化

不是所有 `REJECTED` 都应该自动修复。自动推进只适用于低风险、局部、无外部副作用的问题。

- **可自动执行**：局部代码修复、补测试、文档同步
- **保留到最终确认**：跨模块规则调整、规范升级、删除/迁移、外部副作用操作
- **事实缺失**：如验证入口无法判断，允许中途向用户确认
