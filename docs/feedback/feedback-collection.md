...
| **Agent 反馈** | 自检、Reviewer、Tester | 先记录；阻塞项优先自动修复回流，非阻塞建议最终统一汇总 |
...
2. **记录**：写入 `docs/memory/feedback/agent-feedback.md`，标注自动执行状态与最终汇总状态
3. **分类**：
   - 阻塞型反馈（`REJECTED`）→ 主 agent 自动修复并继续流程
   - 非阻塞建议（`APPROVED` 下的改进项）→ 当前主流程可继续，在最终交付前统一向用户汇总
4. **执行/拒绝**：阻塞项默认自动修复；高风险或不适合自动执行的建议保留到最终总结
...
- `REJECTED` 反馈属于阻塞项，主 agent 在记录后默认自动修复回流；`APPROVED` 下的建议项可以在最终交付前统一汇总
