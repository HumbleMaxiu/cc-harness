...
- 什么是值得构建的 harness
- harness 应如何随项目复杂度演进
- 哪些约束应固化为 scaffold、role skill 契约、workflow、hook 或 check
...
- 优先单执行者 + 清晰工具，而不是默认多角色并行
- 优先预定义 workflow，而不是默认开放式自治
- 只有当单执行者已经因为职责过多、工具混淆、条件分支失控而失效时，才拆分角色

对 `cc-harness` 的含义：

- scaffold 默认提供最小角色集和最小文档集
- 多 role skill 编排是升级路径，不是入门门槛
- 任何新能力都应先回答：“不用新增角色，能否通过更好的 prompt、tool schema、文档结构解决？”
...
harness 的目标不是让 AI 看到更多文档，而是让它更快找到正确层级的信息。
...
- `AGENTS.md`：被用户与 AI 执行者读取，作为导航和规则入口
...
- 用 workflow / role skills 做计划、审查、验证、反馈的流程化收口
...
- 让阻塞型 role/self-check 反馈可自动回流修复
...
只有满足以下至少一项，才应考虑新增 role skill：

- 单执行者 prompt 已出现大量条件分支，难以维护
- 工具集合过多且语义重叠，导致经常选错
- 某职责需要独立门禁，不应与实现者是同一角色
- 该角色输出具有稳定的独立交付物

否则优先保留较少 role skills，通过更清晰的职责文案和 workflow 分段解决。
...
- 我们如何判断 harness 做得更好
- 我们如何决定新增 role skill / docs / checks
- 我们如何把外部最佳实践转成产品能力
...
- 我应该从哪个 workflow 开始
- 什么时候需要升级到更强约束
- 我该如何维护生成出来的 docs / role skills / memory
...
4. 这项复杂度是否真的需要，还是单执行者 / 单 workflow 就够了？
