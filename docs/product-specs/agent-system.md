...
| 模式 | 适用场景 | 主要收益 | 主要代价 | 错误使用信号 |
|------|---------|----------|----------|--------------|
| **Skill 模式** | 单一任务、边界清楚、无需独立 reviewer | 快、上下文集中、适合低风险小变更 | 独立审查弱 | 第二轮反馈、需要独立 reviewer/tester、或完成声明依赖复杂推断 |
| **Subagent 模式** | 需要循环审查、有状态追踪、需要 Challenger 独立挑战 | 状态清楚，Reviewer / Tester / Challenger 门禁明确 | prompt 打包和 handoff 校验成本更高 | 空 handoff 被当通过、Tester 被 build 替代、Challenger Gate 没有落到流程 |
| **Team 模式** | 多角度并行审查或并行验证多个风险面 | 并行覆盖多个独立风险面 | 协调和汇总成本最高 | 只是普通串行审查却开 Team，或没有明确并行问题拆分 |

模式选择时先看收益、代价和错误使用信号，而不是只看任务大小。
...
- `pre-dev-challenge`：位于 `Architect -> Developer` 之间，处理 `plan-claim` 与 `api-assumption`
- `pre-final-challenge`：位于 `Tester -> Final Summary` 之间，处理 `completion-claim` 与 `drift-verification-gap`
- `challenger-result=CONFIRMED` 才能继续主干；`REFUTED` 回到产生 claim 的角色；`UNVERIFIED` 根据 `blocking_threshold` 决定补证据还是作为 remaining risk 汇报
...
Subagent loop 必须维护 `Loop Budget`，避免自动回流无上限：

- `review_loop_count`：Reviewer `REJECTED -> Developer fix` 的次数
- `test_loop_count`：Tester `REJECTED -> Developer fix` 的次数
- `challenge_loop_count`：Challenger `REFUTED/UNVERIFIED -> 补证据/补计划/补验证` 的次数
- 任一计数超过对应上限时设置 `loop_budget_exceeded=true`，停止自动回流并进入 `BLOCKED` 或升级到 Team
