...
const { logHook, parseHookInput } = require('./plan-persist-common');
...
async function main() {
  ...
  if (!shouldRunHook(flags)) {
    logHook('runner', 'skipping hook due to profile gating', {
      event,
      flags,
  ...
  logHook('runner', 'resolved hook script path', {
    event,
    hookScript,
    scriptPath,
  });
  ...
  if (!scriptPath || !fs.existsSync(scriptPath)) {
    ...
    logHook('runner', 'hook script not found; passing through payload', {
      event,
      hookScript,
      scriptPath,
    });
  ...
  logHook('runner', 'hook child process completed', {
    event,
  ...
  if (result.error || result.status === null || result.signal) {
    ...
    logHook('runner', 'hook child process failed', { event, reason });
    process.exit(1);
  }

  logHook('runner', 'hook runner exiting successfully', {
    event,
...
main().catch((error) => {
  ...
  logHook('runner', 'unexpected runner failure', { error: error.message });
  process.exit(1);
});
