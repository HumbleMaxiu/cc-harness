...
def main() -> int:
    ...
    local_head_sha = fetch_local_head_sha(repo_root)
    pr_value = resolve_pr(args.pr, repo_root)
    if pr_value is None:
        return 1
    pr_metadata = fetch_pr_metadata(pr_value, repo_root) or {}
    remote_head_sha = normalize_sha(pr_metadata.get("headRefOid"))
    ...
    external = [summarize_check(c) for c in checks if is_external_check(c)]
    ...
    pending_results = [
        summarize_check(check, local_head_sha=local_head_sha, remote_head_sha=remote_head_sha)
        for check in pending
    ]
    stale_results = [result for result in results if result.get("isStale")]
    ...
    if args.json:
        ...
        render_results(pr_value, results, pending_results=pending_results, stale_results=stale_results)
...
def fetch_local_head_sha(repo_root: Path) -> str:
    result = subprocess.run(
        ["git", "rev-parse", "HEAD"],
        cwd=repo_root,
        text=True,
...
def fetch_pr_metadata(pr_value: str, repo_root: Path) -> dict[str, Any] | None:
    fields = ["number", "url", "headRefOid", "headRefName"]
...
def is_pending(check: dict[str, Any]) -> bool:
    ...
    if state in PENDING_STATES:
        return True
    bucket = normalize_field(check.get("bucket"))
    return bucket in PENDING_BUCKETS


def is_external_check(check: dict[str, Any]) -> bool:
    url = check.get("detailsUrl") or check.get("link") or ""
    return bool(url) and extract_run_id(str(url)) is None
...
def analyze_check(
    ...
    run_head_sha = normalize_sha((metadata or {}).get("headSha"))
    base["runHeadSha"] = run_head_sha
    base["isStale"] = is_stale_run(
...
def summarize_check(
    ...
    return {
        ...
        "detailsUrl": url,
        "runId": extract_run_id(str(url)),
        "jobId": extract_job_id(str(url)),
        "localHeadSha": local_head_sha,
        "remoteHeadSha": remote_head_sha,
    }
...
def is_stale_run(run_head_sha: str, local_head_sha: str, remote_head_sha: str) -> bool:
    if not run_head_sha:
        return False
    expected_sha = remote_head_sha or local_head_sha
    if not expected_sha:
        return False
    return run_head_sha != expected_sha
...
def render_results(
    ...
    pending_results: Iterable[dict[str, Any]] = (),
    stale_results: Iterable[dict[str, Any]] = (),
) -> None:
    results_list = list(results)
    pending_list = list(pending_results)
    stale_list = list(stale_results)
    print(f"PR #{pr_number}: {len(results_list)} failing checks analyzed.")
    if pending_list:
        print(f"Pending checks: {len(pending_list)}")
        for pending in pending_list:
            print(f"  - {pending.get('name', '')}: {pending.get('state') or pending.get('bucket')}")
    if stale_list:
        print(f"Stale failing checks: {len(stale_list)}")
