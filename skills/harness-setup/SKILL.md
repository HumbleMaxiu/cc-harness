...
- 创建 `docs/` 目录树及模板文件
- 暴露默认 harness 能力入口（例如 `/pm-orchestrator`、`/follow-goal`、`/doc-sync`、`/harness-help`、`/harness-audit`、`/harness-quality-gate`）
...
- 已安装 Skills 快速参考（带使用场景；默认包含 `/follow-goal`、`/doc-sync`、`/harness-help`、`/harness-audit`、`/harness-guide`、`/harness-quality-gate`）
...
如果 scaffold 生成了 `pm-orchestrator` 或等效实现/审查 workflow 入口，则应同时在 `AGENTS.md` 或相关指南中暴露 `/doc-sync`，让用户和 agent 都有统一的文档维护入口。
